...
#define MAX_LENGTH_OF_OBJ_LINE 29//for T record. it is the length of obj codes
...
char***obj_file;//read the context of obj_file for loading
...
external_symbol*estab;//external symbol table
int size_of_estab;

typedef struct reference_number{
	int index;
	int address;
}reference_number;
reference_number*ref_num;//saving reference number
int size_of_ref_num;
...
bp_node*bp_head;//linked list for saving breakpoints

//registers for run
int reg[10];//index is register number. ex A:0 X:1...
int num_of_breaks;//how many breakpoints during the run
int end_of_program;//the location of the end of the program
...
//in load.c
void set_progaddr(char*address);//set progaddr to address
void load(int num_of_files,char**filename);//link and load files
int search_estab(char*symbol_name,int*found);//search external symbol table and if found return address, else return 0.
void insert_to_estab(char*symbol_name,int address,char*control_section);//insert symbol to external symbol table
int search_ref_num(int index,int*found);//search address for reference number. if found, return address, else return 0.

//in breakpoint.c
void new_break_point(char*address);//set breakpoint for debuging at address
void clear_break_point(void);//clear every breakpoint
void print_break_point(void);//print every breakpoint
int check_break_point(void);//if PC is on breakpoint, return 1. else, return 0;

//run.c
void run(void);//run program loaded on memory
