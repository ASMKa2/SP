...
int main(void) {
	...
	progaddr=0;//initialize progaddr to 0X00
	obj_file=(char***)malloc(sizeof(char**));
	size_of_estab=0;
	estab=(external_symbol*)malloc(sizeof(external_symbol));
	size_of_ref_num=0;
	ref_num=(reference_number*)malloc(sizeof(reference_number));

	bp_head=(bp_node*)malloc(sizeof(bp_node));//initialize breakpoint linked list
	bp_head->next=NULL;
	...
	while (1) {//loop till user quits
		...
		if (!strcmp(token[0], "type")) {
		...
		else if(!strcmp(token[0], "loader")){
			switch(num_of_token){
				case 2: load(num_of_token-1,token+1);
						break;
				case 3: load(num_of_token-1,token+1);
						break;
				case 4: load(num_of_token-1,token+1);
						break;
				default:printf("Error: \"loader\" command can have 1~3 parameters.\n");
						valid_command=0;
						break;
			}
		}
			...
			if (!strcmp(token[0], "h") || !strcmp(token[0], "help")) {
			...
			else if (!strcmp(token[0], "progaddr")) {
				if(num_of_token==2)
					set_progaddr(token[1]);
				else{
					printf("Error: \"progaddr\" command needs 1 parameter. (address)\n");
					valid_command=0;
				}
			}
			else if (!strcmp(token[0], "bp")) {
				if(num_of_token==1)
					print_break_point();
				else if(num_of_token==2){
					if(!strcmp(token[1],"clear"))
						clear_break_point();
					else
						new_break_point(token[1]);
				}
				else{
					printf("Error: \"bp\" command needs 0~1 parameter. (address/\"clear\")\n");
					valid_command=0;
				}
			}
				...
				if(num_of_token==1)
					...
					printf("Error: \"run\" command does not need a parameter.\n");
					valid_command=0;
				}
			}
			else {
